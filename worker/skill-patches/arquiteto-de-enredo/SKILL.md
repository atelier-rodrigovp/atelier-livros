...
(1) digest **para FATOS**, não documentos integrais (mas a **VOZ/CRAFT** o escritor lê
direto da skill+CRAFT a cada capítulo — voz não se comprime); (2) ledger, não releitura;
(3) revisão cirúrgica, não reescrita; (4) orquestrador enxuto; (5) janela de calibração do
...
- **VOZ vs FATOS (separar os canais — NÃO comprimir a voz):** o `livro-escritor.md`
  gerado deve deixar EXPLÍCITO que o digest do contextualizador é só para
  **FATOS/continuidade** (relógios, pistas, MCL), e que a **VOZ/TÉCNICA** ele lê DIRETO
  da craft a CADA capítulo — NUNCA de um resumo comprimido: o bloco `## CRAFT DA SKILL`
  do perfil **E** os arquivos de craft da `skill_escrita` em
  `~/.claude/skills/<skill_escrita>/references/` (p.ex. `voz-e-oficio.md`,
  `metamodelo-thriller.md`). "Invocar a skill" = LER esses arquivos, não só o resumo do
  `SKILL.md`. A regra "use o digest, não os documentos integrais" vale para FATOS, **não
  para a voz**.
