...
async function continuarCapitulos(skillId: string, totalCaps: number, ctx: CtxCanario): Promise<Record<string, unknown>> {
  ...
  for (let n = 1; n <= totalCaps; n++) {
    ...
    if (cap && (cap.status === "aprovado" || cap.status === "aprovado_com_excecao")) {
      if (revalidar) {
        // Caminho ÚNICO de decisão: a revalidação é o PRÓPRIO pipeline com o texto
        // do disco como base (gates/sinais/revisor/auditor/correções idênticos) —
        // scripts nunca reimplementam decisão de aprovação.
        ...
        if (!ficha) throw new Error(`revalidação do cap ${n}: ficha não encontrada na persistência`);
        const textoBase = await fs.readFile(path.join(deps.dirManuscrito, `capitulo-${String(n).padStart(2, "0")}.md`), "utf8");
        const r = await escreverCapitulo(deps, n, { fichaExistente: ficha, textoBase });
        console.log(`   → ${r.status}${r.problemas.length ? ` (${r.problemas[0]})` : ""} · runs: ${r.runs.length}`);
        resultados.push({ capitulo: n, revalidado: true, status: r.status, textHash: r.textHash, reviewId: r.reviewId, runs: r.runs.length });
